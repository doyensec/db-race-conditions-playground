...
func (self *MySqlDb) init(isolation string) {
	if isolation == "0" {
	...
	db, err := sql.Open("mysql", "racer:password@tcp(localhost:3306)/test001")
	if err != nil {
		panic(err)
	}
	self.db = db
...
func (self *MySqlDb) transfer(request string, source int, destination int, amount int) error {
	opts := sql.TxOptions{
...
func (self *MySqlDb) dump() ([]User, error) {
